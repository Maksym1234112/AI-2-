...
classifier = OneVsOneClassifier(svm.SVC(kernel='poly', degree=8))
classifier.fit(X, y)
...
f1 = cross_val_score(classifier, X, y, scoring='f1_weighted', cv=3)    #   cv - ????   показників якості класифікації??
...
count = 0
error_fix_input_data_not_array = []
for i, item in enumerate(input_data):
    if item.isdigit():
        input_data_encoded[i] = int(input_data[i])
    else:
        error_fix_input_data_not_array.append(input_data[i])
        input_data_encoded[i] = int(label_encoder[count].transform(error_fix_input_data_not_array))
        count += 1
        error_fix_input_data_not_array = []
